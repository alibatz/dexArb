...
contract SwapHelper is IUniswapV3SwapCallback {
    function swap(IUniswapV3Pool pool, bool zeroForOne, int256 amountSpecified, uint160 sqrtPriceLimitX96)
        external
...
contract ArbTest is Test {
    ...
    function test_executeArb() public {
        ...
        bytes32 EVENT_SIG = keccak256("OptimalInputFound(uint256,uint256)");
        for (uint256 i = 0; i < logs.length; i++) {
            if (logs[i].topics[0] == EVENT_SIG) {
                // this is your event
                (uint256 a, uint256 b) = abi.decode(logs[i].data, (uint256, uint256));

                console.log("Optimal Input Found Event:", a, b);
            }
        }
    }
}
